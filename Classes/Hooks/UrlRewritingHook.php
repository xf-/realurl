...
class UrlRewritingHook implements SingletonInterface {
	...
	protected function encodeSpURL_cHashProcessing($newUrl, &$paramKeyValues) {
		...
		if (isset($paramKeyValues['cHash'])) {
			...
			if ($this->rebuildCHash) {
				...
				$cacheHash = ($cacheHashClassExists ? GeneralUtility::makeInstance('TYPO3\\CMS\\Frontend\\Page\\CacheHashCalculator') : NULL);
				...
				if ($cacheHashClassExists) {
					...
					$cHashParameters = GeneralUtility::cHashParams($cHashParameters);
				}
				...
				if (count($cHashParameters) == 1) {
					...
					if ($cacheHashClassExists) {
						...
						/** @noinspection PhpUndefinedMethodInspection PhpDeprecationInspection */
						$paramKeyValues['cHash'] = GeneralUtility::calculateCHash($cHashParameters);
					} else {
						/** @noinspection PhpUndefinedMethodInspection PhpDeprecationInspection */
						$paramKeyValues['cHash'] = GeneralUtility::shortMD5(serialize($cHashParameters));
					}
				}
				unset($cHashParameters);
			}
	...
	protected function decodeSpURL_doDecode($speakingURIpath, $cHashCache = FALSE) {
		...
		if ($cHashCache) {
			...
			if ($cacheHashClassExists) {
				...
				/** @noinspection PhpUndefinedMethodInspection PhpDeprecationInspection */
				$containsRelevantParametersForCHashCreation = count(GeneralUtility::cHashParams($queryString)) > 0;
			}
	...
	protected function setConfig() {
		...
		if ($realUrlConf['enableAutoConf']) {
			$autoConfPath = PATH_site . ConfigurationGenerator::AUTOCONFIGURTION_FILE;
